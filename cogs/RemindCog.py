...
class RemindAgent(commands.Cog):
    ...
    @commands.command()
    async def reminder(self, ctx, *, reminder_text):
        """Set a reminder with a specific date and time. Format: !reminder YYYY-MM-DD HH:MM Your reminder message"""
        pattern = r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}) (.+)"
        match = re.match(pattern, reminder_text)
        
        if not match:
            await ctx.send("Invalid format. Use: !reminder YYYY-MM-DD HH:MM Your reminder message")
            return
    ...
    async def remind(self,ctx,time,*,task):
        ...
        if converted_time == -1:
            await ctx.send("You didn't provide the proper unit")
            return
        elif converted_time == -2:
            await ctx.send("The time must be an integer")
            return
        await ctx.send(f"I will remind you about {task} in {time}")
